...
var barier =[]; // Барьеры
var barierTimer; // для барьера
var BARIER_SPEED = 3000;
...
function move() {
    ...
    if (!isSnakeUnit(new_unit) && new_unit !== undefined && !haveBarier(new_unit)) {
...
function createBarrier() {
    ...
    while (!barierCreated) { //Бесконечный цикл создания препятсвий
        var barierX = Math.floor(Math.random() * FIELD_SIZE_X); //Рандом х
        var barierY = Math.floor(Math.random() * FIELD_SIZE_Y); //Рандом y
